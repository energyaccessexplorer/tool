// Estimate nearest-neighbour oversampling block size from repeated pixel patterns.
export function estimate_oversampling_block_size(data, width, height, nodata) {
...
export async function fetch_unscaled_raster(dataset) {
	const endpoint = (dataset.source_files?.length ? dataset.source_files : dataset.host?.source_files)
		?.find(f => f.func === 'raster')?.endpoint;
	if (!endpoint) return null;

	const url = endpoint.replace(/^(https?):\/\/s3\.amazonaws\.com\/([^/]+)\//, '$1://$2.s3.amazonaws.com/');
